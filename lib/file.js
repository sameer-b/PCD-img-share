...
exports.processFile = function(req, res) {
    fs.readFile(req.file.path, function (err, data) {
        var newPath = `${normalize(__parentDir)}/uploads/${req.cookies.email}/${req.file.filename}`;
...
exports.download = function(req, res) {
    var file = `${normalize(__parentDir)}/uploads/${req.cookies.email}/${req.params.file}`,
        filestream;
...
exports.view = function(req, res) {
    ...
    db.get('files').find({filename: req.params.file}, (err, docs) => {
        if (err) {
            res.render('login');
        } else {
            res.render('view', docs[0]);
        }
    });
};
 ...
 */
var getCmd = function(email, filename, attr_key) {
    return `java -jar ${normalize(__dirname)}/PCDBridge.jar ${normalize(__parentDir)}/keys/pub_key ${normalize(__parentDir)}/keys/${attr_key} ${normalize(__parentDir)}/uploads/${email}/${filename}`;
};

var normalize = function(oldPath) {
...
var executeBridge = function(req, res) {
    var cmd = getCmd(req.cookies.email, req.file.filename, "attr_key_1");
    ...
    exec(cmd, {}, function (error, stdout, stderr) {
        ...
        if (error || stderr) {
            cmd = getCmd(req.cookies.email, req.file.filename, "attr_key_2");
            exec(cmd, {}, function(error, stdout, stderr) {
                console.log('stdout: ' + stdout);
                console.log('stderr: ' + stderr);
                ...
                if (stdout) {
                    addFileEntry(req.cookies.email, req.file.originalname, req.file.filename, 'free');
                    home.renderHome(req, res, {
                        uploadSuccess: "Success! We comply with your policy! However we classify policy as liberal and you will be charged less!"
                    });
                }
            });
        }
        ...
        if(stdout) {
            addFileEntry(req.cookies.email, req.file.originalname, req.file.filename, 'paid');
            home.renderHome(req, res, {
                uploadSuccess: "Success! We comply with your policy! However we classify policy as restrictive and you will be charged more!"
            });
        }
    });
}
 ...
 * @return {Void}
 */
var addFileEntry = function(email, originalname, filename, tier) {
    db.get('files').insert({
        'email': email,
